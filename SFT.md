...
+ `processor/`) + a model card to
`IDEAS-Lab-Northwestern/gr00t-n16-datagen-v1-<fam>-joint-2cam` — the intermediate `checkpoint-*/`
dirs, the training-only `experiment_cfg/` (leaks absolute server paths), and DeepSpeed ZeRO state
are excluded (~6.6 GB, not the ~150 GB full run dir). **wandb**: project
