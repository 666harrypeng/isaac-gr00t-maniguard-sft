...
#   source "$GR00T_HOME/.venv/bin/activate"   # GR00T_HOME defaults to the repo root
#   # ensure FFmpeg libs are on PATH          # torchcodec needs FFmpeg for H.264 decode
...
TAG="${TAG:-}"             # empty by default; e.g. TAG=-run2 to distinguish a run's repo
SFT_DATA_ROOT="${MANIGUARD_SFT_DATA_ROOT:-$REPO_ROOT/outputs/sft_datasets}"
...
run_family() {
  ...
  # 3. train ~EPOCHS epochs (8-card, online wandb)
  bash tools/gr00t_sft/run_sft.sh \
    --dataset "$prepped" --output "$out" \
    --steps "$steps" --batch "$BATCH" --gpus "$GPUS" --lr "$LR" --workers "$WORKERS" --exp-name "$fam"
